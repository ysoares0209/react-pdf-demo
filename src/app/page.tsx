...
const PDFWrapperMemoized = memo(PDFWrapper, (prevProps, nextProps) => {
  return (
    ...
    prevProps.mockedParagraphCount === nextProps.mockedParagraphCount
  );
});
...
export default function Home() {
  ...
  const [mockedParagraphsCount, setMockedParagraphsCount] = useState(0);

  const [debouncedTitle] = useDebounce(title, 1000);
  const [debouncedSubtitle] = useDebounce(subtitle, 1000);

  const addMockedParagraph = () => {
    setMockedParagraphsCount(mockedParagraphsCount + 1);
  };
    ...
    <main className={classes.root}>
      ...
      <div className={classes.sectionWrapper}>
        ...
        <section className={classes.leftSection}>
          ...
          <button onClick={addMockedParagraph}>Add paragraph</button>
        </section>
        {/* LEFT SECTION */}
        <section className={classes.rightSection}>
          <PDFWrapperMemoized
            ...
            mockedParagraphCount={mockedParagraphsCount}
          />
        </section>
      </div>
    </main>
